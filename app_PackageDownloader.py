...
if uploaded_file is not None:
    ...
    secret_patterns = re.compile(r"(password|token|secret|api_key)\s*[=:]", re.IGNORECASE)
    if secret_patterns.search(content):
        st.warning("⚠️ requirements.txt にシークレット（password/token/secret/api_key）の可能性のあるキーワードが含まれています。内容を確認してください。")
